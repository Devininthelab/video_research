...
class DDIMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        if (
            ...
            raise ValueError(
                (
                    ...
                    " `DDIMScheduler.step()` is not supported. Make sure to pass"
        ...
        alpha_prod_t_prev = 1 / (sigma_next**2 + 1)

        # Convert sample (EDM) to z (Standard)
        # z_t = x_t * sqrt(alpha_t)
        z_t = sample * alpha_prod_t**0.5

        # 1. compute predicted original sample (x_0) from sigma-scaled predicted noise
        if self.config.prediction_type == "epsilon":
            pred_original_sample = (z_t - (1 - alpha_prod_t) ** 0.5 * model_output) / alpha_prod_t ** 0.5
            epsilon = model_output
        elif self.config.prediction_type == "v_prediction":
            # v_prediction: v = alpha_t**0.5 * epsilon - (1 - alpha_t)**0.5 * x_0
            # x_0 = alpha_t**0.5 * z_t - (1 - alpha_t)**0.5 * v
            pred_original_sample = alpha_prod_t**0.5 * z_t - (1 - alpha_prod_t)**0.5 * model_output
            epsilon = (1 - alpha_prod_t)**0.5 * z_t + alpha_prod_t**0.5 * model_output
        else:
            raise ValueError(
